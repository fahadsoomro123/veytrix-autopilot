...
/** Native VEYTRIX flagship Android surface, visually keyed to the approved product concept. */
public final class MainActivity extends Activity {
    ...
    private void showComposer() {
        prompt.setVisibility(View.VISIBLE); send.setVisibility(View.VISIBLE); mic.setVisibility(View.VISIBLE);
        positionComposer();
    }
    ...
    private final class VeytrixView extends View {
        ...
        private final Runnable frame = new Runnable() {
            @Override public void run() { pulse += 0.035f; invalidate(); handler.postDelayed(this, 42); }
        };
        ...
        private void drawHome(Canvas c) {
            ...
            drawCore(c,195,205,76);
            text(c,"What should VEYTRIX",21,WHITE,195,322,true); text(c,"accomplish?",21,WHITE,195,348,true);
            text(c,"Your intelligent execution partner.",11,MUTED,195,372,true);
            ...
            text(c,"•",14,WHITE,48,532,true); text(c,"Start typing above...",9,DIM,64,532,false);
            miniStatus(c,26,570,"LOCAL WORKSPACE","READY",GREEN); miniStatus(c,206,570,"AUTOPILOT","ONLINE",GOLD);
        }
        ...
        private void drawDrawer(Canvas c) {
            p.setColor(Color.argb(150,0,0,0)); c.drawRect(0,0,getWidth(),getHeight(),p); round(c,0,0,320,760,Color.rgb(13,17,18),Color.rgb(47,50,46),0);
        ...
        @Override public boolean onTouchEvent(MotionEvent e){if(e.getAction()!=MotionEvent.ACTION_UP)return true;float x=e.getX()/s(),y=e.getY()/s();
            if(drawer){if(x>320){drawer=false;invalidate();return true;}if(y>145&&y<205)select(HOME);else if(y>=205&&y<255)select(RUNS);else if(y>=255&&y<315)select(ARTIFACTS);else if(y>=315&&y<365)select(TOOLS);else if(y>=365&&y<418)select(SETTINGS);else if(y>=418&&y<470)select(VOICE);else if(y>=470&&y<520)select(COMPLETED);return true;}
            if(y<68&&x<70){drawer=true;hideComposer();invalidate();return true;}if(y>680){if(x<78)select(HOME);else if(x<156)select(RUNS);else if(x<234)select(ARTIFACTS);else if(x<312)select(TOOLS);else select(PROFILE);return true;}
