...
if grep -Eqi 'VEYTRIX_KEYSTORE_(BASE64|PASSWORD)|VEYTRIX_KEY_ALIAS|VEYTRIX_KEY_PASSWORD' "$log_file"; then
  ...
  code_pattern='(compilation failed|compile.*error|compileJava.*failed|javac.*error|cannot find symbol|unclosed (string literal|character literal)|illegal start of (expression|type)|reached end of file while parsing|\)[^[:alnum:]]+expected|;[^[:alnum:]]+expected|\}[^[:alnum:]]+expected|[;{}] expected|syntax error|type error|test failed|assertion.*failed|lint.*error|module not found|execution failed|gradle.*failed|build failed|check.*failed)'
  if grep -Eqi "$code_pattern" "$log_file"; then
