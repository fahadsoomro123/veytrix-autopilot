...
for f in package.json pnpm-lock.yaml yarn.lock package-lock.json gradlew build.gradle build.gradle.kts settings.gradle settings.gradle.kts pom.xml pyproject.toml requirements.txt Makefile; do
...
if [[ -f gradlew || -f build.gradle || -f build.gradle.kts ]]; then echo 'gradle-project=yes'; fi
