...
| `NexusNovaAndroid/autopilot/src/main/java/com/nexusnova/autopilot/MainActivity.java` | `android/autopilot/src/main/java/com/veytrix/autopilot/MainActivity.java` | Adapted client | Preserves token encryption, GitHub API, run tracking, artifacts, history, voice, templates and theme; target repo becomes input | MIGRATED + IMPROVED |
| `NexusNovaAndroid/autopilot/src/main/AndroidManifest.xml` | `android/autopilot/src/main/AndroidManifest.xml` | Adapted manifest | Veytrix package/app identity; cleartext disabled; backup disabled | MIGRATED + IMPROVED |
| `NexusNovaAndroid/autopilot/src/main/res/values/styles.xml` | `android/autopilot/src/main/res/values/styles.xml` | Adapted theme | Veytrix resource identity | MIGRATED |
| `NexusNovaAndroid/autopilot/build.gradle.kts` | `android/autopilot/build.gradle.kts` | Adapted module build | Remove NexusNova package/signing coupling; standalone debug/release structure | MIGRATED + IMPROVED |
| `NexusNovaAndroid/autopilot/proguard-rules.pro` | `android/autopilot/proguard-rules.pro` | Migrated statement | Same current requirement | MIGRATED |
...
| Source Autopilot commit history | `docs/migration/*` | Evidence record | Preserve provenance and rationale for adaptation | MIGRATED AS EVIDENCE |

## Mapping rule

No directory was bulk-copied. Each target file has a documented source path, adaptation method, and verification plan. Source remains unchanged and is still the reference implementation until migration completeness is proven.
